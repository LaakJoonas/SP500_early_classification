...
class CALIMERA:
    def __init__(self, delay_penalty, class_weight='balanced'):
        """
        Initialize CALIMERA with delay penalty and class weighting
        
        Parameters:
        - delay_penalty: float, penalty for delay in classification
        - class_weight: dict, 'balanced', or None
                       If dict, should be {1: weight_for_up, 2: weight_for_down}
                       If 'balanced', automatically balance classes (recommended for UP-biased data)
                       If None, no weighting applied
        """
        self.delay_penalty = delay_penalty
        self.class_weight = {1,1.3}
    ...
    def _learn_feature_extractors(X, timestamps):
        ...
        for timestamp in timestamps:
            if timestamp < 9:
                ...
                extractors.append(MiniRocketMultivariate(num_kernels=10000).fit(X_sub).transform)
    ...
    def _learn_classifiers(features, ys, timestamps, class_weight=None):
        T = timestamps.shape[0]
        classifiers = [None for t in range(T)]

        for t in range(T):
            classifier = WeakClassifier(class_weight=class_weight)
    ...
    def fit(self, X_train, labels):
        ...
        # Pass class_weight to classifiers
        self.classifiers = CALIMERA._learn_classifiers(
            features_train, labels, timestamps, class_weight=self.class_weight
        )
...
class WeakClassifier:
    def __init__(self, class_weight=None):
        """
        Initialize WeakClassifier with optional class weighting
        
        Parameters:
        - class_weight: dict, 'balanced', or None
                       If dict, should be {1: weight_for_up, 2: weight_for_down}
                       If 'balanced', automatically balance classes
                       If None, no weighting applied
        """
    ...
    def fit(self, X, y):
        ...
        # Calculate class weights to favor DOWN class (class 2)
        if self.class_weight == 'balanced':
            ...
            for cls, count in zip(unique_classes, class_counts):
                balanced_weights[cls] = n_samples / (n_classes * count)
            
            print(f"Balanced class weights: {balanced_weights}")
            self.computed_class_weight = balanced_weights
        elif isinstance(self.class_weight, dict):
            # Use provided weights
            self.computed_class_weight = self.class_weight
            print(f"Using provided class weights: {self.computed_class_weight}")
        else:
            # No weighting
            self.computed_class_weight = None

        self.feature_means = np.mean(X, axis=0)
        self.feature_norms = np.linalg.norm(X, axis=0)
        self.feature_norms[self.feature_norms == 0] = 1.0

        # Apply class weights to RidgeClassifierCV
        uncalibrated_clf = RidgeClassifierCV(
            alphas=RC_ALPHAS, 
            store_cv_values=True, 
            scoring='accuracy',
            class_weight=self.computed_class_weight  # Add class weights
        )
